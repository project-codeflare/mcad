...
func (r *AppWrapperReconciler) deleteResources(ctx context.Context, appWrapper *mcadv1beta1.AppWrapper, timestamp metav1.Time) bool {
	...
	if appWrapper.Spec.Scheduling.Requeuing.MaxTimeInSeconds == 0 {
	...
	if !metav1.Now().After(timestamp.Add(time.Duration(appWrapper.Spec.Scheduling.Requeuing.MaxTimeInSeconds) * time.Second)) {
